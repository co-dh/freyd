...
| ~~`RegBundle` / `S2_154.SmallRegCat`~~ | **DONE.** The book explicitly names the category of small regular categories in §2.154, so `SmallRegCat` survives. Its structure and instances now live in the minimal leaf `S2_154_SmallRegCat.lean`; capitalization imports that leaf and uses the same bundle, while the full §2.154 equivalence imports it without a cycle. `RegBundle` is removed. |
