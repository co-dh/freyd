...
## Level §2.1 — the Frobenius framework (`functorialSemanticsForRelationalTheories.pdf`)
...
## Level §2.2 — tape diagrams solve union (`TapeDiagrams.pdf`)
...
## Level §2.31 — division *is* diagrammatic (`DiagrammaticAlgebraOfFirstOrderLogic.pdf`)
...
```
CB_Σ                                        the free cartesian bicategory on Σ      §2.1     functorialSemantics
CB_Σ wrapped in tapes  (⊗ inside, ⊕ outside)  + biproduct                            §2.2     TapeDiagrams
CB_Σ + cocartesian + linear = fo-bicategory   + complement, second composition       §2.31    DiagrammaticAlgebraOfFirstOrderLogic
```
...
| -------------------------- | ----------- | ------------------------------------------------------------ |
| `;`, `∩`, `°`, `⊤`, maps   | §2.1        | done, sound, and the fragment is faithful (functorialSemantics §7)     |
| `∪`, `⊥`                   | §2.2        | done and complete for the positive fragment (TapeDiagrams)      |
| `R / S`, `R \ S`           | §2.31       | done and complete, at the cost of leaving the monotone fragment (DiagrammaticAlgebraOfFirstOrderLogic) |
| `Λ`, power transpose       | §2.4        | open                                                          |
| folds, hylos, `μ`          | —           | open — nothing internal to `Rel`; the real gap                |

The allegory-side dictionary for the bottom two rows does not exist yet; §10 of DiagrammaticAlgebraOfFirstOrderLogic lists it as future work.

## Sources

Cited by FILE NAME throughout, never by arXiv id. Every local PDF sits at the repo root, and the short forms used
above are exact prefixes of the file names, so tab completion finds them.

| short form used above                  | file, at the repo root                         | arXiv      |
| -------------------------------------- | ---------------------------------------------- | ---------- |
| `functorialSemantics`                  | `functorialSemanticsForRelationalTheories.pdf` | 1711.08699 |
| `TapeDiagrams`                         | `TapeDiagrams.pdf`                             | 2210.09950 |
| `DiagrammaticAlgebraOfFirstOrderLogic` | `DiagrammaticAlgebraOfFirstOrderLogic.pdf`     | 2401.07055 |
| `GraphicalConjunctiveQueries`          | `GraphicalConjunctiveQueries.pdf`              | 1804.07626 |

| status         | file / reference                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------- |
| local, primary | `functorialSemanticsForRelationalTheories.pdf` — Bonchi, Pavlović, Sobociński (2017)   |
| local, primary | `TapeDiagrams.pdf` — Bonchi, Di Giorgio, Santamaria, *Deconstructing the Calculus of Relations with Tape Diagrams* |
...
| local, primary | `DiagrammaticAlgebraOfFirstOrderLogic.pdf` — Bonchi, Di Giorgio, Haydon, Sobociński (2024) — residuals, complement, complete for FOL |
| local          | `GraphicalConjunctiveQueries.pdf` — Bonchi, Seeber, Sobociński (2018) — the `∃∧` fragment |
...
| unverified     | Olivier & Serrato, *Peirce allegories* (1997) — cited as [58] by DiagrammaticAlgebraOfFirstOrderLogic as the likely allegory counterpart |
