...
public structure Over (B : 𝒞) where
  dom : 𝒞
  hom : dom ⟶ B

public structure OverHom {B : 𝒞} (X Y : Over B) where
  f : X.dom ⟶ Y.dom
  w : f ≫ Y.hom = X.hom

@[ext]
public theorem OverHom.ext {B : 𝒞} {X Y : Over B} {a b : OverHom X Y} (h : a.f = b.f) : a = b := by
  obtain ⟨af, aw⟩ := a; obtain ⟨bf, bw⟩ := b; subst h; rfl

/-- Composition in A/B (explicit, bypasses `Cat` instance). -/
@[expose] public def OverHom.comp {B : 𝒞} {X Y Z : Over B} (h : OverHom X Y) (k : OverHom Y Z) : OverHom X Z :=
...
@[expose] public instance overCat (B : 𝒞) : Cat.{v} (Over B) where
  Hom := OverHom
...
@[expose] public abbrev OverMono {B : 𝒞} {Z Y : Over B} (m : OverHom Z Y) : Prop := @Monic (Over B) _ Z Y m

/-- Iso in A/B: `OverIso m` iff `m` is an isomorphism in the slice category. -/
@[expose] public abbrev OverIso {B : 𝒞} {X Y : Over B} (m : OverHom X Y) : Prop := @IsIso (Over B) _ X Y m
...
public theorem overIso_underlying {B : 𝒞} {X Y : Over B} {m : OverHom X Y} (hm : OverIso m) :
    IsIso m.f :=
  let ⟨inv, h1, h2⟩ := hm
...
public theorem overIso_of_underlying {B : 𝒞} {X Y : Over B} (m : OverHom X Y) (hf : IsIso m.f) :
    OverIso m :=
  let ⟨inv_f, h1, h2⟩ := hf
...
public structure Under (B : 𝒞) where
  cod : 𝒞
  hom : B ⟶ cod

/-- Morphisms of B\A: commuting co-triangles.  `f : UnderHom X Y` is an arrow
    `f.f : X.cod → Y.cod` such that `X.hom ≫ f.f = Y.hom`. -/
public structure UnderHom {B : 𝒞} (X Y : Under B) where
  f : X.cod ⟶ Y.cod
  w : X.hom ≫ f = Y.hom

@[ext]
public theorem UnderHom.ext {B : 𝒞} {X Y : Under B} {a b : UnderHom X Y} (h : a.f = b.f) : a = b := by
  obtain ⟨af, aw⟩ := a; obtain ⟨bf, bw⟩ := b; subst h; rfl

/-- Composition in B\A. -/
@[expose] public def UnderHom.comp {B : 𝒞} {X Y Z : Under B} (h : UnderHom X Y) (k : UnderHom Y Z) :
    UnderHom X Z :=
...
@[expose] public instance underCat (B : 𝒞) : Cat.{v} (Under B) where
  Hom := UnderHom
