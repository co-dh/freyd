/-
  ...
  - B&dM `∋·Q` (`Q` then `∋ = ∈°`) is `Q° ≫ (∋ a)°` — diagram order transposes the pointwise
    pair, exactly as for `minRel` — so `(∋·Q)/∋` is `(((∋ a)°) \ (Q° ≫ (∋ a)°))`.
...
/-- **(8.1)**: `thin Q = (∈\∈) ∩ ((∋·Q)/∋)`, mirrored: `y (thinRel Q) x ⟺ x ⊆ y ∧
    ∀ a ∈ y, ∃ b ∈ x, Q b a` — shrink a set without losing `Q`-lower bounds for any member. -/
...
public theorem thinRel_mono {Q R : a ⟶ a} (h : Q ⊑ R) : thinRel Q ⊑ thinRel R := by
  ...
  exact le_inter (inter_lb_left _ _)
    (le_trans (inter_lb_right _ _)
      (leftDiv_mono_right _ (comp_mono_right (recip_mono h) ((∋ a)°))))
...
public theorem thinRel_comp_minRel_le {Q R : a ⟶ a} (hQR : Q ⊑ R) (htransR : R ≫ R ⊑ R) :
  ...
  · rw [← Cat.assoc]
    have htransR' : R° ≫ R° ⊑ R° := by
      have h := recip_mono htransR; rwa [Allegory.recip_comp] at h
    have s1 : ((∋ a)° ≫ thinRel Q) ≫ minRel R ⊑ (Q° ≫ (∋ a)°) ≫ minRel R :=
      comp_mono_right (recip_eps_comp_thinRel_le Q) (minRel R)
    have s2 : (Q° ≫ (∋ a)°) ≫ minRel R ⊑ R° := by
      rw [Cat.assoc]
      have t1 : Q° ≫ ((∋ a)° ≫ minRel R) ⊑ Q° ≫ R° :=
        comp_mono_left (Q°) (recip_eps_comp_minRel_le R)
...
public theorem thinRel_comp_minRel {Q R : a ⟶ a} (hQR : Q ⊑ R) (hreflQ : Cat.id a ⊑ Q)
...
theorem minRel_comp_singletonMap_le_thinRel (Q : a ⟶ a) :
  ...
  · apply (le_leftDiv_iff _ _ _).mpr
    ...
    have s1 : ((∋ a)° ≫ minRel Q) ≫ singletonMap ⊑ Q° ≫ singletonMap :=
      comp_mono_right (recip_eps_comp_minRel_le Q) singletonMap
...
theorem Λ_comp_minRel_comp_singletonMap_le_thinRel {S : b ⟶ a} {Q R : a ⟶ a}
  ...
  refine ⟨?_, ?_⟩
  · -- `(ΛS ≫ min R ≫ τ) ≫ ∈ ⊑ S`
    rw [Cat.assoc (Λ S) (minRel R ≫ singletonMap) (∋ a),
  ...
  · -- `S° ≫ (ΛS ≫ min R ≫ τ) ⊑ Q° ≫ ∋`
    ...
    rw [← Cat.assoc (Λ S) (minRel R) singletonMap, (Λ_comp_minRel_context S R).symm,
        Cat.assoc (Λ S) (minRel (R ∩ (S° ≫ S))) singletonMap,
        ← Cat.assoc S° (Λ S) (minRel (R ∩ (S° ≫ S)) ≫ singletonMap)]
    have s1 : (S° ≫ Λ S) ≫ (minRel (R ∩ (S° ≫ S)) ≫ singletonMap)
    ...
    have s2 : (∋ a)° ≫ (minRel (R ∩ (S° ≫ S)) ≫ singletonMap) ⊑ Q° ≫ (∋ a)° := by
      ...
      have t1 : ((∋ a)° ≫ minRel (R ∩ (S° ≫ S))) ≫ singletonMap
          ⊑ (R ∩ (S° ≫ S))° ≫ singletonMap :=
        comp_mono_right (recip_eps_comp_minRel_le (R ∩ (S° ≫ S))) singletonMap
      have t2 : (R ∩ (S° ≫ S))° ≫ singletonMap ⊑ Q° ≫ singletonMap :=
        comp_mono_right (recip_mono h) singletonMap
...
/-- **THEOREM 8.1 (the thinning theorem, B&dM p.195)**: for a transitive `Q` and an algebra `S`
    monotonic on `Q` in the mirrored sense (`MonotonicAlg S Q`, i.e. B&dM's "monotonic on `Q°`"),
    thinning at every unfold step (`⦇Λ(F∈·S)·thin Q⦈`) refines thinning once, at the end, on the
    plain catamorphism (`thin Q·Λ⦇S⦈`), mirrored
...
public theorem thinning (hFr : F.PreservesRecip) (I : InitialAlgebra F) {Q : a ⟶ a}
  ...
  · -- (ii) `⦇S⦈°·⦇ΛW·thin Q⦈ ⊑ Q·∋`, by the hylomorphism theorem
    ...
    have b2 : (Q° ≫ S°) ≫ (F.map ((∋ a)°) ≫ (Λ (F.map (∋ a) ≫ S) ≫ thinRel Q))
        ⊑ Q° ≫ (∋ a)° := by
...
/-- **Corollary 8.1 (B&dM p.196)**: thinning at every step, then taking the `R`-minimum, refines
    ...
    `id ⊑ Q`, `Q` and `R` transitive, and `MonotonicAlg S Q`.  Immediate from THEOREM 8.1
...
public theorem thinning_min (hFr : F.PreservesRecip) (I : InitialAlgebra F) {Q R : a ⟶ a}
    ...
    relCata I (Λ (F.map (∋ a) ≫ S) ≫ thinRel Q) ≫ minRel R ⊑ Λ (relCata I S) ≫ minRel R := by
  have hrhs : Λ (relCata I S) ≫ minRel R = (Λ (relCata I S) ≫ thinRel Q) ≫ minRel R := by
...
/-- **Ex 8.6**, the context rule for thin: thinning by `Q` versus thinning by `Q` restricted to
    ...
    `S° ≫ (ΛS ≫ thin Q) ⊑ (Q ∩ S°S)° ≫ ∈°`, and since `S°S` is symmetric that right-hand side
    is `(Q° ∩ S°S) ≫ ∈°`; the intersection under the composite is produced by the MODULAR LAW
    `modular_le` at `(R,S,T) = (Q°, ∈°, S°·(ΛS·thin Q))`, using `(ΛS ≫ thin Q) ≫ ∈ ⊑ S`
    (thinning shrinks) to bound `T ≫ ∈` by `S°S`.  Note the naive "distribute `leftDiv` over the
    numerator meet" route is unavailable — `(Q°∩V)·∈°` does not split as `(Q°·∈°) ∩ (V·∈°)` for
    the lax `∈°`. -/
