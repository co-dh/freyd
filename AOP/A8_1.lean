...
/-- `ΛW·subset = W/∋` mirrored: `Λ W ≫ supset = W / (∋ a)` — the transpose of `W`
    followed by shrinking is exactly "all members come from `W`".  (Ex 7.2's
    `existsImage_comp_supset` is the instance `W := ∋ ≫ R`.) -/
public theorem Λ_comp_supset (W : B ⟶ A) : Λ W ≫ supset = W / (∋ A) := by
  apply le_antisymm
  · apply (le_div_iff _ _ _).mpr
    have h1 : supset ≫ ∋ A ⊑ ∋ A := supset_comp_eps_le
    have h2 : Λ W ≫ (supset ≫ ∋ A) ⊑ Λ W ≫ ∋ A := comp_mono_left _ h1
    rw [Λ_eps_eq'] at h2
    rwa [Cat.assoc]
...
public theorem thinRel_comp_eps_le (Q : A ⟶ A) : thinRel Q ≫ ∋ A ⊑ ∋ A :=
  le_trans (comp_mono_right (inter_lb_left _ _) (∋ A)) supset_comp_eps_le
...
public theorem Λ_comp_thinRel (S : B ⟶ A) (Q : A ⟶ A) :
  ...
  show Λ S ≫ (supset ∩ (((∋ A)°) \ (Q° ≫ (∋ A)°))) = _
  rw [simple_dist_inter (Λ_is_map' S).2, Λ_comp_supset, Λ_comp_lb]
...
public theorem thinRel_trans {Q : A ⟶ A} (htrans : Q ≫ Q ⊑ Q) :
  ...
  show thinRel Q ≫ thinRel Q ⊑ supset ∩ (((∋ A)°) \ (Q° ≫ (∋ A)°))
  refine le_inter ?_ ?_
  · -- component 1: `⊑ supset = (∋a)/(∋a)`
...
public theorem est_comp_singletonMap_le_thinRel (Q : A ⟶ A) :
  ...
  show est Q ≫ singletonMap ⊑ supset ∩ (((∋ A)°) \ (Q° ≫ (∋ A)°))
  refine le_inter ?_ ?_
