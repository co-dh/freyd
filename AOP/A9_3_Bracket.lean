...
public theorem consNE_lax_natural {B : Type} (R : CL.dE A ⟶ CL.dE B) :
    rprodMap R (nelist R) ≫ (CL.consR : (⟨B × NEList B⟩ : RelSet.{0}) ⟶ dNE B)
      ⊑ (CL.consR : (⟨A × NEList A⟩ : RelSet.{0}) ⟶ dNE A) ≫ nelist R := by
  simp only [CL.consR]
  exact graph_lax _ _ _ _ fun p q h => ⟨h.1, h.2⟩
...
/-- `graft≜(mct×mct) bin`: our name, not the book's — the best trees of the two halves grafted
    under one node. -/
@[expose] public def graft :=
...
/-- `loop`, pointwise: `loop(f)(c,[])=c` and `loop(f)(c,y:ys)=loop(f)(f(c,y),ys)`, with `f` a
    relation, since `next` is one. -/
@[expose] public def loopP {X Y : Type} (F : X × Y → X → Prop) : X → CL.ConsList Unit Y → X → Prop
  | c, CL.ConsList.wrap _, w => w = c
  | c, CL.ConsList.cons y ys, w => ∃ c', F (c, y) c' ∧ loopP F c' ys w

/-- `loop(f)`, the Appendix's standard loop: `f` folded along a list from the left. -/
@[expose] public def loop {X : RelSet.{0}} {Y : Type} (F : (⟨X.carrier × Y⟩ : RelSet.{0}) ⟶ X) :
    (⟨X.carrier × CL.ConsList Unit Y⟩ : RelSet.{0}) ⟶ X :=
  fun p w => loopP F p.1 p.2 w
...
@[expose] public def process :
    (⟨A × CL.ConsList Unit (CL.ConsList Unit (Tree A))⟩ : RelSet.{0}) ⟶ dList (Tree A) :=
...
public theorem loop_char {X Z W : Type} {Y : Type} (k : X × NEList Y → Z) (g : X → Z) (h : NEList Y → W)
    (F : (⟨Z × W⟩ : RelSet.{0}) ⟶ (⟨Z⟩ : RelSet.{0}))
    (hnil : ∀ a b w, F (g a, h (CL.ConsList.wrap b)) w ↔ w = k (a, CL.ConsList.wrap b))
  ...
  have key : ∀ (a : X) (z u : NEList Y) (w : Z),
    ...
    | wrap b =>
      intro u w
      show (∃ c', F (k (a, u), h (cat u (CL.ConsList.wrap b))) c' ∧ w = c') ↔ _
      exact ⟨fun ⟨c', hc, hw⟩ => hw.trans ((hsnoc a u b c').mp hc),
        fun hw => ⟨w, (hsnoc a u b w).mpr hw, rfl⟩⟩
    | cons b z ih =>
      intro u w
  ...
  cases x with
  | wrap b =>
    show (∃ c', F (g a, h (CL.ConsList.wrap b)) c' ∧ w = c') ↔ _
    exact ⟨fun ⟨c', hc, hw⟩ => hw.trans ((hnil a b c').mp hc),
  ...
  | cons b z =>
    ...
    exact ⟨fun ⟨c', hc, hl⟩ => by rwa [(hnil a b c').mp hc] at hl,
      fun hl => ⟨_, (hnil a b _).mpr rfl, hl⟩⟩
...
public theorem col_rec_at
  ...
  constructor
  · intro hn
    obtain ⟨_, ⟨rfl, _⟩, hw⟩ := (Iff.of_eq e').mpr ⟨_, ⟨rfl, a, z, rfl⟩, _, rfl, hn⟩
    exact hw
  · intro hw
    obtain ⟨_, ⟨rfl, _⟩, _, rfl, hn⟩ := (Iff.of_eq e').mp ⟨_, ⟨rfl, a, z, rfl⟩, hw⟩
    exact hn
...
/-- `tail` on lists, `[]` its own `tail`.  Irreducible, as `tailFn` is. -/
@[expose, irreducible] public def listTailFn {X : Type} : CL.ConsList Unit X → CL.ConsList Unit X
  | CL.ConsList.wrap u => CL.ConsList.wrap u
  | CL.ConsList.cons _ xs => xs

public theorem listTailFn_cons {X : Type} (x : X) (xs : CL.ConsList Unit X) :
    listTailFn (CL.ConsList.cons x xs) = xs := by unfold listTailFn; rfl

public theorem listTailFn_wrap {X : Type} (u : Unit) :
    listTailFn (CL.ConsList.wrap u : CL.ConsList Unit X) = CL.ConsList.wrap u := by unfold listTailFn; rfl

/-- `tail list(f)=list(f) tail`, pointwise. -/
public theorem cmap_listTail {X Y : Type} (f : X → Y) (xs : CL.ConsList Unit X) :
    cmap f (listTailFn xs) = listTailFn (cmap f xs) := by
  cases xs with
  | wrap u =>
    show cmap f (listTailFn (CL.ConsList.wrap u)) = listTailFn (CL.ConsList.wrap u)
    rw [listTailFn_wrap, listTailFn_wrap]; rfl
  | cons x xs =>
...
public theorem graph_rpair {C D P Q : RelSet.{0}} (f : C.carrier → D.carrier) (U : D ⟶ P) (V : D ⟶ Q) :
    graph f ≫ rpair U V = rpair (graph f ≫ U) (graph f ≫ V) := by
  apply hom_ext; intro x w
  constructor
  · rintro ⟨m, hm, hU, hV⟩; exact ⟨⟨m, hm, hU⟩, ⟨m, hm, hV⟩⟩
  · rintro ⟨⟨m, hm, hU⟩, ⟨m', hm', hV⟩⟩
...
omit [Inhabited A] in
/-- The `tic` bead is lax natural. -/
public theorem tic_lax_natural {B : Type} (R : CL.dE A ⟶ CL.dE B) :
...
omit [Inhabited A] in
/-- The `⟨π₁ wrap,tic⟩` bead is lax natural. -/
public theorem outlWrap_consInitsTail_lax_natural {B : Type} (R : CL.dE A ⟶ CL.dE B) :
    rprodMap R (nelist R)
        ≫ rpair ((graph Prod.fst : (⟨B × NEList B⟩ : RelSet.{0}) ⟶ dA B) ≫ (CL.wrapR : dA B ⟶ dNE B))
          (tic (A := B))
      ⊑ rpair ((graph Prod.fst : (⟨A × NEList A⟩ : RelSet.{0}) ⟶ dA A) ≫ (CL.wrapR : dA A ⟶ dNE A))
          (tic (A := A))
        ≫ rprodMap (nelist R) (list (nelist R)) := by
...
set_option hygiene false in
local notation "tailTG" => (graph listTailFn : dList (Tree A) ⟶ dList (Tree A))
set_option hygiene false in
local notation "outlN" => (graph Prod.fst : (⟨A × NEList A⟩ : RelSet.{0}) ⟶ dA A)
set_option hygiene false in
local notation "outrN" => (graph Prod.snd : (⟨A × NEList A⟩ : RelSet.{0}) ⟶ dNE A)
set_option hygiene false in
local notation "outlA" => (graph Prod.fst
  : (⟨A × CL.ConsList Unit (CL.ConsList Unit (Tree A))⟩ : RelSet.{0}) ⟶ dA A)
set_option hygiene false in
local notation "outrA" => (graph Prod.snd
  : (⟨A × CL.ConsList Unit (CL.ConsList Unit (Tree A))⟩ : RelSet.{0})
...
@[expose] public def step :
    (⟨A × CL.ConsList Unit (CL.ConsList Unit (Tree A))⟩ : RelSet.{0})
      ⟶ dList (CL.ConsList Unit (Tree A)) :=
  rpair (process st sb cb ≫ tailTG) outrA ≫ zipC ≫ listConsT

/-- `addcol≜⟨π₁ tip wrap,step⟩ cons`. -/
@[expose] public def addcol :
    (⟨A × CL.ConsList Unit (CL.ConsList Unit (Tree A))⟩ : RelSet.{0})
      ⟶ dList (CL.ConsList Unit (Tree A)) :=
  rpair (outlA ≫ tipG ≫ CL.singleR ()) (step st sb cb) ≫ consC
...
/-- `tops`, first step: definition of `tops`. -/
public theorem tops_step1 : tops st sb cb = tic ≫ list (graph (mct st sb cb) : dNE A ⟶ dTree A) := rfl

/-- `tops`, second step: `tail list(f)=list(f) tail`, and definition of `col`. -/
public theorem tops_step2 :
    tic ≫ list (graph (mct st sb cb) : dNE A ⟶ dTree A) = consNE ≫ col st sb cb ≫ tailTG := by
  simp only [tic, col, CL.consR, list_graph, graph_comp, cmap_listTail]

/-- `tops`, third step: (9.9). -/
public theorem tops_step3 :
    consNE ≫ col st sb cb ≫ tailTG = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ process st sb cb ≫ tailTG := by
  rw [← Cat.assoc, col_cons st sb cb, Cat.assoc]

/-- `tops=(𝟙×array) process tail`. -/
public theorem tops_rec : tops st sb cb = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ process st sb cb ≫ tailTG :=
  (tops_step1 st sb cb).trans <| (tops_step2 st sb cb).trans (tops_step3 st sb cb)
...
public theorem rests_eq : rests st sb cb = outrN ≫ array st sb cb :=
  (rests_step1 st sb cb).trans (rests_step2 st sb cb)

/-- `newrows≜tic list(row)`: our name, not the book's — the rows of the new array below its first. -/
@[expose] public def newrows : (⟨A × NEList A⟩ : RelSet.{0}) ⟶ dList (CL.ConsList Unit (Tree A)) :=
  tic ≫ list (row st sb cb)

/-- `newrows`, first step: definition of `newrows`. -/
public theorem newrows_step1 : newrows st sb cb = tic ≫ list (row st sb cb) := rfl

/-- `newrows`, second step: (9.10) on the non-singleton `tic` lists. -/
...
/-- `newrows`, fourth step: the `tops` and `rests` equations; products; definition of `step`. -/
public theorem newrows_step4 :
    rpair (tops st sb cb) (rests st sb cb) ≫ zipC ≫ listConsT
      = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ step st sb cb := by
  rw [tops_rec, rests_eq, step, idArray_graph, array_graph]
  simp only [← Cat.assoc, graph_rpair, graph_comp]

/-- `newrows=(𝟙×array) step`: the rows below the first are `step` of `a` and the array of `x`. -/
public theorem newrows_rec : newrows st sb cb = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ step st sb cb :=
  (newrows_step1 st sb cb).trans <| (newrows_step2 st sb cb).trans <|
    (newrows_step3 st sb cb).trans (newrows_step4 st sb cb)

/-- `addcol`, third step: `cons list(f)=(f×list f) cons`, and definition of `newrows`. -/
...
public theorem array_cons_step4 :
    rpair (outlN ≫ wrapNE ≫ row st sb cb) (newrows st sb cb) ≫ consC
      = rpair (outlN ≫ tipG ≫ CL.singleR ()) (newrows st sb cb) ≫ consC := by
...
public theorem array_cons_step5 :
    rpair (outlN ≫ tipG ≫ CL.singleR ()) (newrows st sb cb) ≫ consC
      = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ addcol st sb cb := by
  rw [newrows_rec, addcol, idArray_graph]
  simp only [← Cat.assoc, graph_rpair, graph_comp]
...
public theorem array_cons :
    consNE ≫ array st sb cb = rprodMap (𝟙 (dA A)) (array st sb cb) ≫ addcol st sb cb :=
  (array_cons_step1 st sb cb).trans <| (array_cons_step2 st sb cb).trans <|
    (array_cons_step3 st sb cb).trans <| (array_cons_step4 st sb cb).trans (array_cons_step5 st sb cb)
