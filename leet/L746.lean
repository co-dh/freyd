...
/-- **Honest headline (§7.5 `min (≤)·Λ spec`)**: `solve` is exactly the morphism `Λ spec ≫ est D`
    for `D w z := w ≤ z` — `est` of the reversed "smaller-is-better" order is the `≤`-MINIMUM
    — not merely pointwise. Bridged from `solve_correct`. -/
theorem solve_eq_est : solve = Λ spec ≫ est (fun w z : Int => w ≤ z) :=
  eq_Λ_comp_est _ (fun x y h1 h2 => Int.le_antisymm h1 h2) solveFn spec
