...
// The bead is a POINT and every arm into one is a bend (IntroString.pdf p. 40), so instead of a flat
// bar reaching across, the wire the bead does not consume dips to the dot at `yb` and comes back out.
#let ddip(xo, h, x, y0, y1, yb) = {
  let (t, b) = (if y0 == "top" { h } else { y0 }, if y1 == "bot" { 0 } else { y1 })
  // Two knees now share the run above the dot and one takes the run below, so each is clamped under
  // half its own share: a knee sized as `dlane`'s would turn back up, and that flattens a tangent.
  let k = calc.min(dknee(x, xo, t - yb), 0.45 * (t - yb), 0.45 * (yb - b))
  hm-wire((if y0 == "top" { ((x, h),) } else { ((xo, t), (x, t - k)) })
    + ((x, yb + k), (xo, yb), (x, yb - k))
    + (if y1 == "bot" { ((x, 0),) } else { ((x, b + k), (xo, b)) }))
}
...
#let dpanel(h, w, xo, lanes, beads, top, bot, names: false, s: 74%, dip: (), cert: (:)) = {
  dpan(h, w, xo, {
  for l in lanes {
    let bd = beads.find(bd => dip.contains(bd.at(0)) and bd.at(3, default: none) == l.at(0)
      and (if l.at(1) == "top" { h } else { l.at(1) }) > bd.at(0)
      and (if l.at(2) == "bot" { 0 } else { l.at(2) }) < bd.at(0))
    if bd == none { dlane(xo, h, l.at(0), l.at(1), l.at(2), l.at(3), l.at(4)) }
    else { ddip(xo, h, l.at(0), l.at(1), l.at(2), bd.at(0)) }
  }
  for b in beads {
