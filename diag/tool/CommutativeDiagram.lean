...
/-- What stands BETWEEN one panel and the next: the join's own operator where the statement is about
    a join, `none` everywhere else — one entry per gap, so the row's separators are positional and
    no panel carries the operator in a label. -/
def typstSeps (ss : Array (Option String)) : String :=
  typstArr (ss.toList.map fun s => match s with | some x => typstString x | none => "none") "\n"
...
def cdPage (sel : String) (ps : Array Panel) (seps : Array (Option String) := #[]) : String :=
  let head := "#import \"../../cdpanel.typ\": *\n\n"
  ...
  | _ =>
    head ++ "#let panels = " ++ typstPanels ps ++ "#let seps = " ++ typstSeps seps ++ cert
...
/-- AN ASSERTION ABOUT A JOIN IS ONE CLAIM PER OPERAND.  Every `@[diag_join]` application in the
    ...
    two panels the note sets its `∪` between.  `none` where the statement has no join, which is
    every other statement.

    The head is BETA-REDUCED before the test and not after: a family the statement built is applied
    at an object (`(fun x => φ x ∪ ψ x) B`), and the join is inside the redex.

    The MEET is not tagged and so is not here: `φ∩ψ` is ONE bead on ONE panel, which is what
    `laxNatural_inter_false` makes it — the two operators have the same type and the attribute is
    the whole of the difference. -/
def joinSplit? (body : Expr) : MetaM (Option (String × Expr × Expr)) := do
  let symRef ← IO.mkRef (none : Option String)
  let sub (i : Nat) : MetaM Expr := Meta.transform body (pre := fun x => do
    match ← StrDiag.joinOperands? x.headBeta with
    | some (sym, l, r) => symRef.set (some sym); return .visit (if i == 0 then l else r)
    | none => return .continue)
  let b₀ ← sub 0
  match ← symRef.get with
  | none => return none
