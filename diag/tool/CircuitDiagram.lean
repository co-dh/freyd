...
def objApply (f : String) (a : Obj) : String := applyLabel f a.label a.join
...
partial def objOf (o : Expr) : MetaM Obj := do
  ...
  | (``Freyd.Functor.obj, args) =>
    ...
    | some (f, b) => do
      -- A COMBINATOR RELATOR'S ACTION IS THE OBJECT IT REDUCES TO (`relatorObj?`): the wire carries
      -- `V×X`, not `(V×𝟙)(X)`.  `relatorObj?` INSTANTIATES first, because a lane the reader BUILT
      -- carries metavariables and a head test on the raw expression reads `?m`.
      if let some v ← StrDiag.relatorObj? f o then return ← objOf v
  ...
  | _ => do
    ...
    if ob.kind == .sum then
      ...
      if h : ss.size = 1 then
        ...
        return .mk (objApply "F" c) .sum ob.parts (applyJoin "F")
      return ob
...
partial def pushRecip? (r : Expr) : MetaM (Option Expr) := do
  ...
  | _ =>
    ...
    if let some v ← StrDiag.rewriteHead? r then
      unless ← StrDiag.reopens r v do return some (← rc v)
...
/-- The selectors applied to the side, innermost last.  `.body` opens the least fixed point's
    binder with a local of the binder's own name and draws the body under it — the same body the
    `mu` clause draws, now as the panel itself — so a row whose Hinze–Marsden cell names
    `<decl>.lhs.body` has a circuit cell naming the SAME string.  It takes a continuation's shape
    for the same reason the string route's does: the local has to be in scope while the picture is
    built, since the boxes are labelled from the `Expr`. -/
partial def withSel (sel : List StrDiag.Sel) (e : Expr) : MetaM Val := do
  match sel with
  | [] => panel e
  | [.inl] => return ports (← armOf e 0)
  | [.inr] => return ports (← armOf e 1)
  | .body :: rest =>
    let some φ := StrDiag.muArg? e
      | throwError "`.body` names the body of a least fixed point, and \
          `{← Meta.ppExpr e}` is not one"
    Meta.lambdaBoundedTelescope φ 1 fun xs b => do
      unless xs.size == 1 do
        throwError "`{← Meta.ppExpr φ}` binds no arrow, so `.body` opens no wire to draw \
          the body on"
      withSel rest b
  -- A tape draws the fork itself, so one arm selector is as deep as this route goes: what a second
  -- would name — an operand of the arm — is a Hinze–Marsden panel's question.
  | _ => throwError "a circuit draws ONE arm of the fork at the head of its run, and \
      `{String.join (sel.map (·.suffix))}` names more"
