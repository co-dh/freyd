...
/-- A CONVERSE WITH A NAME OF ITS OWN (CLAUDE.md): the membership's is `∈`, the subset order's
    (`subset ≜ ∋/∋` relates a set to its subsets) is `⊆`, and a `°` makes the reader undo one level
...
def namedRecip (r : Expr) : Option String :=
  ...
  else if r.isAppOf ``Freyd.Alg.subset then some "⊆" else none
