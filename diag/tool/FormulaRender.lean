...
def file (declName : Name) (binder : Option String) (path : List String)
  ...
  let some ci := (← getEnv).find? declName | throwError "no such declaration: {declName}"
  let sel := declName.toString ++ ((binder.map fun h => "#" ++ h).getD "")
    ++ path.foldl (fun a s => a ++ "." ++ s) "" ++ branch.foldl (fun a s => a ++ s.suffix) ""
  return "// GENERATED by `diag-export --formula` — do not edit; regenerate with\n\
    //   ./scripts/diag-export --formula " ++ sel ++ "\n\
    // cert: (lean: \"" ++ declName.toString ++ "@" ++ Freyd.TypeRender.hex8 (← Freyd.TypeRender.stmtKey ci) ++ "\")\n`"
    ++ text ++ "`\n"
